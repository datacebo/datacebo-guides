...
<Steps>
  <Step title="Train a generative model by providing metadata and real data ">
    You use the software system and create a generative model. To create this generative model you provide the software the metadata and a sample of the real data. 
  </Step>
  <Step title="Sample synthetic data from the generative model ">
    Once you have trained a model you can generate as much synthetic data as you want. The data will have the statistical properties of your real data and emulate its basic structures - referential integrity and data formats. 
  </Step>
  <Step title="Post process and use the data ">
    In case you have some specific rules that the data must follow, for example column A must have lower values than column B you could examine the generated data and post process and remove invalid rows. 
  </Step>
</Steps>
...
The amount of manual effort required here is the work required to train the generative model using the software. Different software systems have different level of maturity. Depending upon the maturity your effort maybe minimal. This area is rapidly evolving with our own software system bringing a level of automation to be able to train a generative model on multi table datasets previously unimaginable. 
...
| ----------------------------------------------- | ------------------------------------------------------------------------ | -------------------- | ---------------- |
| Mock data generation                            | Type 1: No information content data                                      | Very high            | No               |
| Rule based data generation                      | Type 2: Heuristically corrected data                                     | Very high            | No               |
| Using LLMs to generate data                     | Type 1 / Type 2: No information content and some hueristically corrected | Very High            | Yes              |
| Building your own generative model              | Type 3: Information rich data                                            | Low                  | Yes              |
| Building constraints augmented generative model | Type 4: Information rich coformative data.                               | Very Low             | Yes              |