...
| Address the sprawl of data types | Be able to model and synthesize data across a wide range of types—not only traditional statistical variables like numerical and categorical fields, but also complex real-world data such as phone numbers, addresses, vehicle identification numbers (VINs), GPS coordinates, and many others. |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Be able to scale to many tables  | A synthetic data platform [<u>must be able to model 100s of interconnected tables</u>](https://datacebo.com/blog/multi-table-synthesizers/)  in order to be useful for enterprise environments.                                                                                                 |
...
| Generate flexible formats        | Be able to model and generate data with flexible formats. For example, if the address is split across multiple columns or if a datetime format is very unique.                                                                                                                                  |

<Note>
  Most generative modeling systems attempt to address the first two. For addressing data types many provide some native integration with faker, but produce data that has no correlations, or 
</Note>
